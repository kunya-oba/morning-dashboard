...
## Git運用ルール
- **自動コミット**: 意味のある変更（機能追加、改善、バグ修正、ドキュメント更新）を完了したら、自動的に `git commit` を実行すること。
- **コミットメッセージ**: 変更内容を分析し、わかりやすい日本語で記述すること。主な変更点を箇条書きで記載し、Claude Codeによる生成であることを明記すること。
- **コミットタイミング**:
  - 新機能の実装が完了したとき
  - バグ修正を行ったとき
  - 複数ファイルにわたる変更が1つのまとまった作業として完了したとき
  - README.md、CLAUDE.md などのドキュメント更新を行ったとき
- **コミットしない場合**: 調査やデバッグのための一時的な変更、ユーザーが明示的に「コミットしないで」と指示した場合
- **プッシュ**: `git push` は自動実行せず、ユーザーが手動で実行する。必要に応じてプッシュを促すこと。
